// This is the profiler for GPUs, make sure you have CUDA and CUPTI installed in your machine.
// This is a prototype, not fully planned and built out yet. 
// To run this, I've included a toy vector addition kernel. You can run it using the following command: 
      // nvcc -o profiled_kernel kernel.cu profiler.cpp -lcupti
      // ./profiled_kernel
...
static void printActivity(CUpti_Activity *record)
{
  ...
  // CUPTI_ACTIVITY_KIND_MEMCPY tracks memory transfers between host and device
  // and we track the following ones for simplicity; 
          // bytes
          // deviceId
          // contId  
  // This section is not done yet. for some reason its not tracking the number of bytes copied but is able to 
  // get the type of memcpy happening
